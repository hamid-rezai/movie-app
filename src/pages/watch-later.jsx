...
const WatchLater = () => {
  const { watchLaterItem, removeItemFromWatchLater } = UseAddToWatchLater();
  ...
  return (
    <div className='bg-navBack relative p-4'>
      ...
      <div className='w-full h-[300px] lg-h-[350px] bg-gradient-to-b from-navBack z-20 flex items-center justify-center gap-2'>
        ...
        <MdAccessTimeFilled className='text-main-color w-[40px] h-[40px] sm:w-[55px] sm:h-[55px]' />
      </div>
      <div className='relative min-w-screen min-h-screen bg-navBack grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-7 p-4 justify-items-center'>
        {watchLaterItem.length > 0 ? (
          watchLaterItem.map((movie) => (
            <div
              ...
              <div className='absolute top-0 right-0'>
                <MdCancel
                  onClick={(e) => {
                    e.stopPropagation();
                    removeItemFromWatchLater(movie);
                  }}
                  className='text-2xl text-slate-200 hover:text-white hover:scale-110 transition-all duration-200'
                />
              </div>
              {/* Movie Image */}
              <img
